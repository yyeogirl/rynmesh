...
核心功能已有历史实测证据，当前代码回归通过，可以提交独立分支评审；整个需求尚未完成最终验收，不能作为当前可用的发行版本交付。

2026-09-18 现场复验：本地 API 可访问，但 `qwen` 请求返回 HTTP 503
`model_alias_target_unavailable`；服务器 SSH 端口 23 连接超时，节点日志也记录
注册服务访问超时。只能确认当前无法从本机访问远端节点，尚不能判断是服务器
停机还是网络故障。恢复连接后须重跑真实 SDK 与 P2P 验收。
...
| 当前现场 SDK 复验 | 未通过：qwen 目标不可用（503），本轮未完成任何 SDK 推理检查；见 final-audit.json |
